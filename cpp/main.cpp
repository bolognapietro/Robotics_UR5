...
int main(int argc, char **argv) {
    ...
    RowVector3d posStandard {{0.4, 0.2, -0.73}};
    ...
    while(ros::ok()) {
        ...
        if (check_point(pos, jointBraccio)) {
            ...
            if (!rot) {
                ...
                // CLOSE GRIPPER
                closeGripper(Th);
                jointBraccio = Th.row(299).block<1, 6>(0, 0);
                ...
                jointBraccio = Th.row(599).block<1, 6>(0, 0);
                cond[1] = threep2p(jointBraccio, posStandard, phiZero, 0, 3, Th);
                // OPEN GRIPPER
                openGripper(Th);
            }
...
RowVector6d getJointBraccio(){
    ...
    RowVector6d jointBraccio {{joints(4), joints(0), joints(3), joints(5), joints(6), joints(7)}};
    return jointBraccio;
}
...
void publish(Matrix8d& Th){
    ros::NodeHandle n;
    ros::Publisher pub = n.advertise<std_msgs::Float32MultiArray>("/moves", 10);
