...
def create_spawnmodel_request(name: str, model: str, position: tuple, rot: int):
    ...
    # ELEMENTARI
        ...
        q = quaternion_from_euler(1.571,0,0) #
    elif (rot==2): #davanti
        q = quaternion_from_euler(-1.571,0,0) #
    elif (rot==3): #sinistra
        q = quaternion_from_euler(1.571,0,1.571) #
    elif (rot==4): #destra
        q = quaternion_from_euler(1.571,0,-1.571) #

    # 2 ROTAZIONI
    elif (rot==5): #2+3
        q = quaternion_from_euler(0,1.571,0) #
    elif (rot==6): #2+4
        q = quaternion_from_euler(0,-1.571,0) #
    elif (rot==7): #3+1
        q = quaternion_from_euler(1.571,1.571,0) #
    elif (rot==8): #3+2
        q = quaternion_from_euler(-1.571,1.571,0) #
    elif (rot==9): #1+1
        q = quaternion_from_euler(3.141,0,0) #
    elif (rot==10): #3+3
        q = quaternion_from_euler(3.141,0,1.571) #
...
def spawn_model(path: str):
    ...
    x = float(input("x: "))
    y = float(input("y: "))
    position = (x+0.5,y+0.35, 0.93)

    rot=int(input("rot: "))

    request = create_spawnmodel_request(name = name, model = model, position = position, rot = rot)
